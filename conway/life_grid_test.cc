...
TEST(LifeGrid, partial_out_of_bounds) {
  LifeGrid grid(3, 2), payload({{1, 1}, {1, 1}});
  LifeGrid empty(grid);

  grid = empty;
  EXPECT_EQ(grid.Load(payload, {-2, -2}), false) << "did not place";
  EXPECT_EQ(grid, empty)
      << "the 2x2 placed at (-2, -2) is completely out of bounds";

  grid = empty;
  EXPECT_EQ(grid.Load(payload, {-1, -1}), false) << "did not place";
  EXPECT_EQ(grid, empty)
      << "the 2x2 block placed at (-1, -1) is partially out of bounds";
  ...
  grid = empty;
  EXPECT_EQ(grid.Load(payload, {2, 1}), false) << "did not place";
  EXPECT_EQ(grid, empty)
      << "the 2x2 block placed at (2, -1) is partially out of bounds";
  ...
  grid = empty;
  EXPECT_EQ(grid.Load(payload, {1, 0}), true) << "should've placed";
  EXPECT_EQ(grid, LifeGrid({{0, 1, 1}, {0, 1, 1}}))
      << "the 2x2 block placed at (1, 0) is in bounds";
}
...
TEST(LifeGrid, istream_extraction) {
  ...
  for (int i = 0; i < 3; ++i) {
    for (int j = 0; j < 3; ++j) {
      EXPECT_EQ(grid.GetCell(Vector2(i, j)),
                expected[j][i] ? LifeState::ALIVE : LifeState::DEAD);
    }
  }
}
